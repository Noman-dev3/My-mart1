...
import { signInUser } from '@/lib/auth-client-actions'; // Updated import
...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginFormValues) => {
    ...
    if (!result.success) {
      ...
      // The AuthProvider will detect the state change and handle the UI update.
  ...
  const GoogleIcon = (props: React.SVGProps<SVGSVGElement>) => (
    <svg viewBox="0 0 48 48" {...props}>
      ...
      <path
        fill="#1976D2"
        d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.574l6.19,5.238C39.99,36.596,44,31.1,44,24C44,22.659,43.862,21.35,43.611,20.083z"
      ></path>
    </svg>
  );
      ...
      <div className="w-full max-w-4xl grid lg:grid-cols-2 shadow-2xl overflow-hidden rounded-2xl z-20 bg-transparent lg:bg-card">
        <div className="p-8 sm:p-12 flex flex-col justify-center bg-card/80 backdrop-blur-md lg:bg-card lg:backdrop-blur-none rounded-t-2xl lg:rounded-l-2xl lg:rounded-tr-none">
          <Link href="/" className="w-fit">
            <Button
              variant="outline"
              className="rounded-full px-4 mb-8 text-muted-foreground group"
            >
              <Icons.logo className="h-5 w-5 mr-2 text-primary group-hover:text-primary transition-colors" />{' '}
              My Mart
            </Button>
          </Link>
          ...
          <Form {...form}>
            ...
            >
              ...
              <Button
                type="submit"
                className="w-full font-bold mt-4 h-12 rounded-xl"
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-card/80 lg:bg-card px-2 text-muted-foreground">
                Or continue with
              </span>
            </div>
          </div>
          ...
          <div className="mt-8 text-center text-sm">
            <p className="text-muted-foreground">
              ...
              <Link
                href="/signup"
                className="underline text-primary font-semibold hover:text-primary/80"
              >
                Sign up
              </Link>
            </p>
